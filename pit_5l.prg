...
PROCEDURE Pit_5L( _G, _M, _STR, _OU )
   ...
   PRIVATE P51, P51a, P51b, P52, P54, P541
   PRIVATE P67, P68
   ...
   STORE 0 TO P39, P40, P41, P46, P47, P48, P51, P51a, P51b, P52
   STORE 0 TO P67, P68
   _czy_close := .F.
      ...
      CASE _OU == 'D'
         ...
         CASE _STR == 3
            FOR x := 1 TO _G
               rl()
            NEXT
            rl( Space( _M ) + Space( 11 ) + dzial_g[ 1, 1 ] + Space( 18 ) + dzial_g[ 1, 2 ] )
            rl()
            rl( Space( _M ) + Space( 4 ) + PadC( dzial_g[ 1, 3 ], 35 ) + Space( 2 ) + PadC( AllTrim( dzial_g[ 1, 4 ] ), 34 ) )
            rl()
            rl( Space( _M ) + Space( 9 ) + tran( dzial_g[ 1, 5 ], '999' ) + Space( 5 ) + tran( dzial_g[ 1, 6 ], picfe ) + Space( 1 ) + tran( dzial_g[ 1, 7 ], picfe ) + Space( 4 ) + tran( dzial_g[ 1, 8 ], picf ) + Space( 3 ) + tran( dzial_g[ 1, 9 ], picf ) )
            rl()
            rl( Space( _M ) + Space( 11 ) + dzial_g[ 2, 1 ] + Space( 18 ) + dzial_g[ 2, 2 ] )
            rl()
            rl( Space( _M ) + Space( 4 ) + PadC( dzial_g[ 2, 3 ], 35 ) + Space( 2 ) + PadC( AllTrim( dzial_g[ 2, 4 ] ), 34 ) )
            rl()
            rl( Space( _M ) + Space( 9 ) + tran( dzial_g[ 2, 5 ], '999' ) + Space( 5 ) + tran( dzial_g[ 2, 6 ], picfe ) + Space( 1 ) + tran( dzial_g[ 2, 7 ], picfe ) + Space( 4 ) + tran( dzial_g[ 2, 8 ], picf ) + Space( 3 ) + tran( dzial_g[ 2, 9 ], picf ) )
            rl()
            rl( Space( _M ) + Space( 11 ) + dzial_g[ 3, 1 ] + Space( 18 ) + dzial_g[ 3, 2 ] )
            rl()
            rl( Space( _M ) + Space( 4 ) + PadC( dzial_g[ 3, 3 ], 35 ) + Space( 2 ) + PadC( AllTrim( dzial_g[ 3, 4 ] ), 34 ) )
            rl()
            rl( Space( _M ) + Space( 9 ) + tran( dzial_g[ 3, 5 ], '999' ) + Space( 5 ) + tran( dzial_g[ 3, 6 ], picfe ) + Space( 1 ) + tran( dzial_g[ 3, 7 ], picfe ) + Space( 4 ) + tran( dzial_g[ 3, 8 ], picf ) + Space( 3 ) + tran( dzial_g[ 3, 9 ], picf ) )
            rl()
            rl( Space( _M ) + Space( 11 ) + dzial_g[ 4, 1 ] + Space( 18 ) + dzial_g[ 4, 2 ] )
            rl()
            rl( Space( _M ) + Space( 4 ) + PadC( dzial_g[ 4, 3 ], 35 ) + Space( 2 ) + PadC( AllTrim( dzial_g[ 4, 4 ] ), 34 ) )
            rl()
            rl( Space( _M ) + Space( 9 ) + tran( dzial_g[ 4, 5 ], '999' ) + Space( 5 ) + tran( dzial_g[ 4, 6 ], picfe ) + Space( 1 ) + tran( dzial_g[ 4, 7 ], picfe ) + Space( 4 ) + tran( dzial_g[ 4, 8 ], picf ) + Space( 3 ) + tran( dzial_g[ 4, 9 ], picf ) )
         ENDCASE
      CASE _OU == 'P'
         rl( PadC( 'PIT-5L   DEKLARACJA NA ZALICZK&__E. MIESI&__E.CZN&__A. NA PODATEK DOCHODOWY', 80 ) )
         rl( PadC( 'WED&__L.UG STAWKI 19%', 80 ) )
         ...
         rl()
         rl( PadC( 'D. ODLICZENIE OD DOCHODU', 80 ) )
         rl( PadC( '========================', 80 ) )
         rl( '(23) Straty z lat ubieglych.........................................' + tran( P51a, dRPIC ) )
         rl( '(23a)Inne odliczenia................................................' + tran( P51b, dRPIC ) )
         ...
         rl( '-----------------------' )
         rl( '(28) Podatek od podstawy z pozycji 27 obliczony wg zasad z art.30c..' + tran( P79, dRPIC ) )
         rl()
         rl( 'G.2. ODLICZENIE OD PODATKU' )
         rl( '--------------------------' )
         rl( '(29) Skladka na ubezp.zdrow.op&_l.acona od poczatku roku...............' + tran( p80, dRPIC ) )
         rl()
