...
FUNCTION edek_vat12_4( aDane )
   LOCAL r, nl, tmp_cel
      ...
      nl = Chr(13) + Chr(10)
      r = '<?xml version="1.0" encoding="UTF-8"?>' + nl
      r = r + '<Deklaracja  xmlns="http://crd.gov.pl/wzor/2018/05/17/5378/" xmlns:etd="http://crd.gov.pl/xml/schematy/dziedzinowe/mf/2016/01/25/eD/DefinicjeTypy/">' + nl
      r = r + '  <Naglowek>' + nl
      r = r + '    <KodFormularza kodPodatku="VAT" kodSystemowy="VAT-12 (4)" rodzajZobowiazania="Z" wersjaSchemy="1-0E" >VAT-12</KodFormularza>' + nl
      r = r + '    <WariantFormularza>4</WariantFormularza>' + nl
		r = r + '    <CelZlozenia poz="P_7">' + tmp_cel + '</CelZlozenia>' + nl
      r = r + '    <Rok>' + AllTrim(p5b) + '</Rok>' + nl
      r = r + '    <Miesiac>' + AllTrim(p5a) + '</Miesiac>' + nl
...
//         ENDIF
      ...
      r = r + '  <PozycjeSzczegolowe>' + nl
		...
		//r = r + '    <P_14>' + TKwotaC( aDane[ 'P_14' ] ) + '</P_14>' + nl
      IF Len(AllTrim(zDEKLTEL)) > 0
   		r = r + '    <P_18>' + AllTrim(zDEKLTEL) + '</P_18>' + nl
      ENDIF
		//r = r + '    <P_19>' + date2strxml(Date()) + '</P_19>' + nl
		r = r + '  </PozycjeSzczegolowe>' + nl
		r = r + '  <Pouczenia>1</Pouczenia>' + nl
      IF ( tmp_cel = '2' .AND. Len(AllTrim(tresc_korekty_vat7)) > 0 )
         r = r + '  <Zalaczniki>' + nl
...
FUNCTION edek_viudo_1( aDane )
   ...
   IF ! Empty( aDane[ 'sekcja_c2' ] ) .OR. ! Empty( aDane[ 'sekcja_c3' ] ) .OR. ! Empty( aDane[ 'sekcja_c5' ] )
      ...
      r += '  <Pouczenie2>' + str2sxml( 'W przypadku niewpˆacenia w obowi¥zuj¥cym terminie kwoty podatku VAT nale¾nej Rzeczpospolitej Polskiej lub wpˆacenia jej w niepeˆnej wysoko˜ci, niniejsza deklaracja stanowi podstaw© do wystawienia tytuˆu wykonawczego, zgodnie z art. 3a õ 1 pkt 1 ustawy z dnia 17 czerwca 1966 r. o post©powaniu egzekucyjnym w administracji (Dz. U. z 2020 r. poz. 1427, z p¢«n. zm.).' ) + '</Pouczenie2>' + nl
      r += '</Deklaracja>' + nl
   ENDIF
...
FUNCTION edek_ift2_12( aDane, lRocznie )

   LOCAL r, nl := Chr( 13 ) + Chr( 10 )

   r := '<?xml version="1.0" encoding="UTF-8"?>' + nl
   IF lRocznie
      r += '<Deklaracja xmlns="http://crd.gov.pl/wzor/2025/10/27/13953/">' + nl
   ELSE
      r += '<Deklaracja xmlns="http://crd.gov.pl/wzor/2025/10/27/13948/">' + nl
   ENDIF
   r += '  <Naglowek>' + nl
   IF lRocznie
      r += '    <KodFormularza kodPodatku="CIT" kodSystemowy="IFT-2R (12)" rodzajZobowiazania="Z" wersjaSchemy="1-0E">IFT-2/IFT-2R</KodFormularza>' + nl
   ELSE
      r += '    <KodFormularza kodPodatku="CIT" kodSystemowy="IFT-2 (12)" rodzajZobowiazania="Z" wersjaSchemy="1-0E">IFT-2/IFT-2R</KodFormularza>' + nl
   ENDIF
   r += '    <WariantFormularza>12</WariantFormularza>' + nl
   r += '    <CelZlozenia poz="P_7">' + aDane[ 'korekta' ] + '</CelZlozenia>' + nl
   r += '    <OkresOd poz="P_4">' + date2strxml( aDane[ 'data_od' ] ) + '</OkresOd>' + nl
   r += '    <OkresDo poz="P_5">' + date2strxml( aDane[ 'data_do' ] ) + '</OkresDo>' + nl
   r += '    <KodUrzedu>' + AllTrim( aDane[ 'Firma' ][ 'KodUrzedu' ] ) + '</KodUrzedu>' + nl
   ...
   IF aDane[ 'D11D' ] <> 0 .OR. aDane[ 'D11E' ] <> 0 .OR. aDane[ 'D11G' ] <> 0
      ...
      r += '    </P_O>' + nl
   ENDIF
   IF aDane[ 'D12D' ] <> 0 .OR. aDane[ 'D12E' ] <> 0 .OR. aDane[ 'D12G' ] <> 0
      r += '    <P_69>' + TKwotaC( aDane[ 'D12D' ] ) + '</P_69>' + nl
      r += '    <P_70>' + TKwotaC( aDane[ 'D12E' ] ) + '</P_70>' + nl
      r += '    <P_71>' + TKwotaC( aDane[ 'D12G' ] ) + '</P_71>' + nl
      r += '    <P_P typ="G">' + nl
      r += '      <P_P1>' + TProcentowy( aDane[ 'D12F' ] ) + '</P_P1>' + nl
      r += '      <P_P2>' + TKwotaC( aDane[ 'D12D' ] ) + '</P_P2>' + nl
      r += '      <P_P3>' + TKwotaC( aDane[ 'D12E' ] ) + '</P_P3>' + nl
      r += '      <P_P4>' + TKwotaC( aDane[ 'D12G' ] ) + '</P_P4>' + nl
      r += '    </P_P>' + nl
   ENDIF
   IF lRocznie
      r += xmlNiePusty( aDane[ 'K01' ], '    <P_72>' + TKwotaC( aDane[ 'K01' ] ) + '</P_72>' + nl )
      r += xmlNiePusty( aDane[ 'K02' ], '    <P_73>' + TKwotaC( aDane[ 'K02' ] ) + '</P_73>' + nl )
      r += xmlNiePusty( aDane[ 'K03' ], '    <P_74>' + TKwotaC( aDane[ 'K03' ] ) + '</P_74>' + nl )
      r += xmlNiePusty( aDane[ 'K04' ], '    <P_75>' + TKwotaC( aDane[ 'K04' ] ) + '</P_75>' + nl )
      r += xmlNiePusty( aDane[ 'K05' ], '    <P_76>' + TKwotaC( aDane[ 'K05' ] ) + '</P_76>' + nl )
      r += xmlNiePusty( aDane[ 'K06' ], '    <P_77>' + TKwotaC( aDane[ 'K06' ] ) + '</P_77>' + nl )
      r += xmlNiePusty( aDane[ 'P01' ], '    <P_78>' + TKwotaC( aDane[ 'P01' ] ) + '</P_78>' + nl )
      r += xmlNiePusty( aDane[ 'P02' ], '    <P_79>' + TKwotaC( aDane[ 'P02' ] ) + '</P_79>' + nl )
      r += xmlNiePusty( aDane[ 'P03' ], '    <P_80>' + TKwotaC( aDane[ 'P03' ] ) + '</P_80>' + nl )
      r += xmlNiePusty( aDane[ 'P04' ], '    <P_81>' + TKwotaC( aDane[ 'P04' ] ) + '</P_81>' + nl )
      r += xmlNiePusty( aDane[ 'P05' ], '    <P_82>' + TKwotaC( aDane[ 'P05' ] ) + '</P_82>' + nl )
      r += xmlNiePusty( aDane[ 'P06' ], '    <P_83>' + TKwotaC( aDane[ 'P06' ] ) + '</P_83>' + nl )
      r += xmlNiePusty( aDane[ 'K07' ], '    <P_84>' + TKwotaC( aDane[ 'K07' ] ) + '</P_84>' + nl )
      r += xmlNiePusty( aDane[ 'K08' ], '    <P_85>' + TKwotaC( aDane[ 'K08' ] ) + '</P_85>' + nl )
      r += xmlNiePusty( aDane[ 'K09' ], '    <P_86>' + TKwotaC( aDane[ 'K09' ] ) + '</P_86>' + nl )
      r += xmlNiePusty( aDane[ 'K10' ], '    <P_87>' + TKwotaC( aDane[ 'K10' ] ) + '</P_87>' + nl )
      r += xmlNiePusty( aDane[ 'K11' ], '    <P_88>' + TKwotaC( aDane[ 'K11' ] ) + '</P_88>' + nl )
      r += xmlNiePusty( aDane[ 'K12' ], '    <P_89>' + TKwotaC( aDane[ 'K12' ] ) + '</P_89>' + nl )
      r += xmlNiePusty( aDane[ 'P07' ], '    <P_90>' + TKwotaC( aDane[ 'P07' ] ) + '</P_90>' + nl )
      r += xmlNiePusty( aDane[ 'P08' ], '    <P_91>' + TKwotaC( aDane[ 'P08' ] ) + '</P_91>' + nl )
      r += xmlNiePusty( aDane[ 'P09' ], '    <P_91>' + TKwotaC( aDane[ 'P09' ] ) + '</P_91>' + nl )
      r += xmlNiePusty( aDane[ 'P10' ], '    <P_93>' + TKwotaC( aDane[ 'P10' ] ) + '</P_93>' + nl )
      r += xmlNiePusty( aDane[ 'P11' ], '    <P_94>' + TKwotaC( aDane[ 'P11' ] ) + '</P_94>' + nl )
      r += xmlNiePusty( aDane[ 'P12' ], '    <P_95>' + TKwotaC( aDane[ 'P12' ] ) + '</P_95>' + nl )
      r += xmlNiePusty( aDane[ 'K13' ], '    <P_96>' + TKwotaC( aDane[ 'K13' ] ) + '</P_96>' + nl )
      r += xmlNiePusty( aDane[ 'K14' ], '    <P_97>' + TKwotaC( aDane[ 'K14' ] ) + '</P_97>' + nl )
      r += xmlNiePusty( aDane[ 'K15' ], '    <P_98>' + TKwotaC( aDane[ 'K15' ] ) + '</P_98>' + nl )
      r += xmlNiePusty( aDane[ 'K16' ], '    <P_99>' + TKwotaC( aDane[ 'K16' ] ) + '</P_99>' + nl )
      r += xmlNiePusty( aDane[ 'K17' ], '    <P_100>' + TKwotaC( aDane[ 'K17' ] ) + '</P_100>' + nl )
      r += xmlNiePusty( aDane[ 'K18' ], '    <P_101>' + TKwotaC( aDane[ 'K18' ] ) + '</P_101>' + nl )
      r += xmlNiePusty( aDane[ 'P13' ], '    <P_102>' + TKwotaC( aDane[ 'P13' ] ) + '</P_102>' + nl )
      r += xmlNiePusty( aDane[ 'P14' ], '    <P_103>' + TKwotaC( aDane[ 'P14' ] ) + '</P_103>' + nl )
      r += xmlNiePusty( aDane[ 'P15' ], '    <P_104>' + TKwotaC( aDane[ 'P15' ] ) + '</P_104>' + nl )
      r += xmlNiePusty( aDane[ 'P16' ], '    <P_105>' + TKwotaC( aDane[ 'P16' ] ) + '</P_105>' + nl )
      r += xmlNiePusty( aDane[ 'P17' ], '    <P_106>' + TKwotaC( aDane[ 'P17' ] ) + '</P_106>' + nl )
      r += xmlNiePusty( aDane[ 'P18' ], '    <P_107>' + TKwotaC( aDane[ 'P18' ] ) + '</P_107>' + nl )
      r += xmlNiePusty( aDane[ 'K19' ], '    <P_108>' + TKwotaC( aDane[ 'K19' ] ) + '</P_108>' + nl )
      r += xmlNiePusty( aDane[ 'K20' ], '    <P_109>' + TKwotaC( aDane[ 'K20' ] ) + '</P_109>' + nl )
      r += xmlNiePusty( aDane[ 'K21' ], '    <P_110>' + TKwotaC( aDane[ 'K21' ] ) + '</P_110>' + nl )
      r += xmlNiePusty( aDane[ 'K22' ], '    <P_111>' + TKwotaC( aDane[ 'K22' ] ) + '</P_111>' + nl )
      r += xmlNiePusty( aDane[ 'K23' ], '    <P_112>' + TKwotaC( aDane[ 'K23' ] ) + '</P_112>' + nl )
      r += xmlNiePusty( aDane[ 'P19' ], '    <P_114>' + TKwotaC( aDane[ 'P19' ] ) + '</P_114>' + nl )
      r += xmlNiePusty( aDane[ 'P20' ], '    <P_115>' + TKwotaC( aDane[ 'P20' ] ) + '</P_115>' + nl )
      r += xmlNiePusty( aDane[ 'P21' ], '    <P_116>' + TKwotaC( aDane[ 'P21' ] ) + '</P_116>' + nl )
      r += xmlNiePusty( aDane[ 'P22' ], '    <P_117>' + TKwotaC( aDane[ 'P22' ] ) + '</P_117>' + nl )
      r += xmlNiePusty( aDane[ 'P23' ], '    <P_118>' + TKwotaC( aDane[ 'P23' ] ) + '</P_118>' + nl )
      r += '    <P_113>' + TKwotaC( aDane[ 'KR' ] ) + '</P_113>' + nl
      r += '    <P_119>' + TKwotaC( aDane[ 'PR' ] ) + '</P_119>' + nl
      r += '    <P_120>' + TNaturalny( aDane[ 'miesiace' ] ) + '</P_120>' + nl
   ENDIF
   IF ! lRocznie .AND. ! Empty( aDane[ 'data_zl' ] )
      r += '    <P_121>' + date2strxml( aDane[ 'data_zl' ] ) + '</P_121>' + nl
   ENDIF
   r += '    <P_122>' + date2strxml( aDane[ 'data_prz' ] ) + '</P_122>' + nl
   r += '  </PozycjeSzczegolowe>' + nl
   r += '  <Pouczenie>1</Pouczenie>' + nl
   r += '</Deklaracja>' + nl
