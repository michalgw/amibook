...
*/
   ...
   ELSE
      ...
      IF ! Empty( nRok ) .AND. ! Empty( nMiesiac ) .AND. ( Val( aDane[ 'Naglowek' ][ 'Rok' ] ) != nRok .OR. Val( aDane[ 'Naglowek' ][ 'Miesiac' ] ) != nMiesiac )
         ...
         RETURN aRes
      ENDIF
      IF ! Empty( nRok ) .AND. Val( aDane[ 'Naglowek' ][ 'Rok' ] ) != nRok
         aRes[ 'Komunikat' ] := 'Bˆ©dny rok danych w pliku (odcytany rok: ' + aDane[ 'Naglowek' ][ 'Rok' ]
         RETURN aRes
      ENDIF
...
PROCEDURE JPKImp_VatS_Dekretuj_FA( aDane )
   ...
   AEval( aDane[ 'JPK' ][ 'Faktura' ], { | aPoz |
      ...
      IF ( ( hb_HHasKey( aPoz, 'P_13_1' ) .AND. aPoz[ 'P_13_1' ] <> 0 ) .OR. ;
         ...
         ( zVATOKRESDR == 'T' .OR. ( ( ( Month( aPoz[ 'P_1' ] ) == Val( miesiac ) ) .AND. ( Year( aPoz[ 'P_1' ] ) == Val( param_rok ) ) ) ;
...
/*         IF hb_HHasKey( aPoz, 'KodWaluty' ) .AND. aPoz[ 'KodWaluty' ] <> 'PLN'
         ...
         aPozDek[ 'AmiRejData' ] := NIL
         aPozDek[ 'AmiKsgData' ] := NIL
         aPozDek[ 'DataWystawienia' ] := aPoz[ 'P_1' ]
         aPozDek[ 'DataSprzedazy' ] := HGetDefault( aPoz, 'P_6', aPoz[ 'P_1' ] )
         aPozDek[ 'DataKSeF' ] := NIL
         ...
         aPozDek[ 'P_15' ] := HGetDefault( aPoz, 'P_15', 0 )
...
PROCEDURE JPKImp_VatS_Dekretuj_VAT( aDane )
   ...
   AEval( aDane[ 'JPK' ][ 'Sprzedaz' ], { | aPoz |
      ...
      IF ( ( hb_HHasKey( aPoz, 'K_10' ) .AND. aPoz[ 'K_10' ] <> 0 ) .OR. ;
         ...
         ( zVATOKRESDR == 'T' .OR. ( ( Month( aPoz[ 'DataWystawienia' ] ) == Val( miesiac ) .AND. ;
...
*/
      ...
      aPozDek[ 'AmiRejData' ] := NIL
      aPozDek[ 'AmiKsgData' ] := NIL
      aPozDek[ 'DataWystawienia' ] := aPoz[ 'DataWystawienia' ]
      aPozDek[ 'DataSprzedazy' ] := aPoz[ 'DataSprzedazy' ]
      aPozDek[ 'DataKSeF' ] := NIL
...
*/
      ...
      aPozDek[ 'DataWystawienia' ] := aPoz[ 'DataWystawienia' ]
      aPozDek[ 'DataSprzedazy' ] := aPoz[ 'DataSprzedazy' ]
      aPozDek[ 'DataKSeF' ] := NrKSeFData( HGetDefault( aPoz, 'NrKSeF', '' ) )
...
PROCEDURE JPKImp_VatZ_Dekretuj_V7( aDane )
   ...
   AEval( aDane[ 'JPK' ][ 'Zakup' ], { | aPoz |
      ...
      aPozDek[ 'AmiRejData' ] := NIL
      aPozDek[ 'AmiKsgData' ] := NIL
      aPozDek[ 'DataWystawienia' ] := aPoz[ 'DataZakupu' ]
      aPozDek[ 'DataSprzedazy' ] := aPoz[ 'DataWplywu' ]
      aPozDek[ 'DataKSeF' ] := NrKSeFData( HGetDefault( aPoz, 'NrKSeF', '' ) )
...
FUNCTION JPKImp_VatS_Importuj( aDane )
   ...
   AEval( aDane[ 'Dekret' ], { | aPoz |
      ...
      IF lImportuj
         ...
         */
         DO CASE
         CASE ! KosEmptyDate( aPoz[ 'AmiRejData' ] )
            zDZIEN := Str( Day( aPoz[ 'AmiRejData' ] ), 2 )
            zMC := Str( Month( aPoz[ 'AmiRejData' ] ), 2 )
         CASE aDane[ 'DataRej' ] == 'W'
            zDZIEN := Str( Day( aPoz[ 'DataWystawienia' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataWystawienia' ] ), 2 )
         CASE aDane[ 'DataRej' ] == 'S' .OR. ( aDane[ 'DataRej' ] == 'K' .AND. KosEmptyDate( aPoz[ 'DataKSeF' ] ) )
            zDZIEN := Str( Day( aPoz[ 'DataSprzedazy' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataSprzedazy' ] ), 2 )
         CASE aDane[ 'DataRej' ] == 'K' .AND. ! KosEmptyDate( aPoz[ 'DataKSeF' ] )
            zDZIEN := Str( Day( aPoz[ 'DataKSeF' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataKSeF' ] ), 2 )
         OTHERWISE
            zDZIEN := Str( Day( aPoz[ 'DataSprzedazy' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataSprzedazy' ] ), 2 )
         ENDCASE
         ...
         DO CASE
         CASE ! KosEmptyDate( aPoz[ 'AmiKsgData' ] )
            zDATAKS := aPoz[ 'AmiKsgData' ]
         CASE aDane[ 'DataKsg' ] == 'W'
            zDATAKS := aPoz[ 'DataWystawienia' ]
         CASE aDane[ 'DataKsg' ] == 'S' .OR. ( aDane[ 'DataKsg' ] == 'K' .AND. KosEmptyDate( aPoz[ 'DataKSeF' ] ) )
            zDATAKS := aPoz[ 'DataSprzedazy' ]
         CASE aDane[ 'DataKsg' ] == 'K' .AND. ! KosEmptyDate( aPoz[ 'DataKSeF' ] )
            zDATAKS := aPoz[ 'DataKSeF' ]
         OTHERWISE
            zDATAKS := aPoz[ 'DataSprzedazy' ]
         ENDCASE
         ...
         zUE := aPoz[ 'zue' ]
         zKRAJ := aPoz[ 'zkraj' ]
         zSEK_CV7 := iif( aDane[ 'IgnorujAd' ] == 'T', '  ', aPoz[ 'zsek_cv7' ] )
         zRACH := 'F'
         zDETAL := DETALISTA
   ...
   AEval( aDane[ 'Dekret' ], { | aPoz |
      ...
      IF lImportuj
         ...
         */
         DO CASE
         CASE ! KosEmptyDate( aPoz[ 'AmiKsgData' ] )
            zDZIEN := Str( Day( aPoz[ 'AmiKsgData' ] ), 2 )
            zMC := Str( Month( aPoz[ 'AmiKsgData' ] ), 2 )
         CASE aDane[ 'DataKsg' ] == 'W'
            zDZIEN := Str( Day( aPoz[ 'DataWystawienia' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataWystawienia' ] ), 2 )
         CASE aDane[ 'DataKsg' ] == 'S' .OR. ( aDane[ 'DataKsg' ] == 'K' .AND. KosEmptyDate( aPoz[ 'DataKSeF' ] ) )
      ...
      IF lImportuj
         ...
         */
         ...
         CASE ! KosEmptyDate( aPoz[ 'AmiKsgData' ] )
            zDZIEN := Str( Day( aPoz[ 'AmiKsgData' ] ), 2 )
            zMC := Str( Month( aPoz[ 'AmiKsgData' ] ), 2 )
         CASE aDane[ 'DataKsg' ] == 'W' .OR. ( aDane[ 'DataKsg' ] == 'K' .AND. KosEmptyDate( aPoz[ 'DataKSeF' ] ) )
            zDZIEN := Str( Day( aPoz[ 'DataWystawienia' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataWystawienia' ] ), 2 )
         CASE aDane[ 'DataKsg' ] == 'Z'
            zDZIEN := Str( Day( aPoz[ 'DataSprzedazy' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataSprzedazy' ] ), 2 )
         CASE aDane[ 'DataKsg' ] == 'K' .AND. ! KosEmptyDate( aPoz[ 'DataKSeF' ] )
            zDZIEN := Str( Day( aPoz[ 'DataKSeF' ] ), 2 )
      ...
      IF lImportuj
         ...
         */
         ...
         CASE ! KosEmptyDate( aPoz[ 'AmiRejData' ] )
            zDZIEN := Str( Day( aPoz[ 'AmiRejData' ] ), 2 )
            zMC := Str( Month( aPoz[ 'AmiRejData' ] ), 2 )
         CASE aDane[ 'DataRej' ] == 'W' .OR. ( aDane[ 'DataRej' ] == 'K' .AND. KosEmptyDate( aPoz[ 'DataKSeF' ] ) )
            zDZIEN := Str( Day( aPoz[ 'DataWystawienia' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataWystawienia' ] ), 2 )
         CASE aDane[ 'DataRej' ] == 'Z'
            zDZIEN := Str( Day( aPoz[ 'DataSprzedazy' ] ), 2 )
            zMC := Str( Month( aPoz[ 'DataSprzedazy' ] ), 2 )
         CASE aDane[ 'DataRej' ] == 'K' .AND. ! KosEmptyDate( aPoz[ 'DataKSeF' ] )
            zDZIEN := Str( Day( aPoz[ 'DataKSeF' ] ), 2 )
...
FUNCTION JPKImp_VatS_Ilosc( aDane )
   ...
   CASE aDane[ 'JPK' ][ 'Naglowek' ][ 'KodFormularza' ] == 'JPK_PKPIR'
...
PROCEDURE JPKImp_VatS( nCelImportu, lZKos )
   ...
   LOCAL aDane := hb_Hash( 'ZezwolNaDuplikaty', 'N', 'Rejestr', '  ', ;
      ...
      'DataRej', param_ksse, 'Oznaczenie', Space( 16 ), 'Procedura', Space( 32 ), ;
      'SprawdzRegon', iif( olparam_ra, 'T', 'N' ), 'RodzDow', Space( 6 ), ;
      'Export', 'N', 'ZezwolNaPuste', 'N', 'DataKsg', param_kssk, 'IgnorujAd', 'N' )
   LOCAL cPlik
   LOCAL cKolor
   LOCAL cEkran := SaveScreen()
   LOCAL nMenu, cEkran2
   LOCAL aRaport, cRaport, cTN, cRej, lOk, cKolKs, cDataRej, cRegon, cExport
   LOCAL cPuste, cDataKsg, cIgnorujAd
   ...
   IF HB_ISHASH( aDane ) .AND. hb_HHasKey( aDane, 'JPK' ) .AND. HB_ISHASH( aDane[ 'JPK' ] )
      ...
      DO WHILE nMenu != 0
         ...
         CASE nMenu == 3
            ...
            @  4, 13 CLEAR TO 22, 72
            @  5, 15 TO 21, 70
            @  6, 17 SAY "Zezw¢l na import dokument¢w z istniej¥cym nr" GET cTN PICTURE "!" VALID ValidTakNie( cTN, 6, 63 )
            @  7, 17 SAY "Domy˜lny symbol rejestru" GET cRej PICTURE "!!" VALID { || Kat_Rej_Wybierz( @cRej, 7, 42 ), .T. }
            @  8, 17 SAY "Opis zdarzenia" GET cOpisZd VALID JPKImp_VatS_Tresc_V( iif( nCelImportu == 1, "S", "R" ) )
            IF zRYCZALT == 'T'
               @  9, 17 SAY "Domy˜lna kol. ewid. (5,6,7,8,9,10,11,12,13)" GET cKolRej PICTURE '@K 99' WHEN Eval( bKolRyczW ) VALID ( AllTrim( cKolRej ) $ '56789' .OR. cKolRej == '10' .OR. cKolRej == '11' .OR. cKolRej == '12' .OR. cKolRej == '13' ) .AND. Eval( bKolRyczV )
            ELSE
            ...
            @ 15, 17 SAY "Pobieraj dane kontrahenta z bazy REGON" GET cRegon PICTURE '!' WHEN olparam_ra VALID ValidTakNie( cRegon, 15, 57 )
            ...
            @ 18, 17 SAY "Ignoruj adnotacje (o.o. i SP)" GET cIgnorujAd PICTURE "!" VALID ValidTakNie( cIgnorujAd, 18, 48 )
            @ 20, 52 GET lOk PUSHBUTTON CAPTION ' Zamknij ' STATE { || ReadKill( .T. ) }
            ValidTakNie( cTN, 6, 63 )
            ValidTakNie( cRegon, 15, 57 )
            ValidTakNie( cExport, 16, 47 )
            ValidTakNie( cPuste, 17, 59 )
            ValidTakNie( cIgnorujAd, 18, 48 )
            READ
            IF LastKey() <> K_ESC
...
PROCEDURE JPKImp_PKPIR()
   ...
   LOCAL aDane := hb_Hash( 'ZezwolNaDuplikaty', 'N', 'OpisZd', Space( 30 ), 'ZezwolNaPuste', 'N' )
   LOCAL cPlik
   LOCAL cKolor
   LOCAL cEkran := SaveScreen()
   LOCAL nMenu, cEkran2
   LOCAL aRaport, cRaport, cTN, lOk, cPuste
   LOCAL nSumaImp, nLiczbaLp := 0
      ...
      IF HB_ISHASH( aDane[ 'JPK' ] )
         ...
         DO WHILE nMenu != 0
            ...
            CASE nMenu == 1
               IF TNEsc( , "Czy wykona† import danych sprzeda¾y? ( T / N )" )
                  ...
                  IF aRaport[ 'Pominieto' ] > 0
                     ...
                     cRaport += "-------------------" + hb_eol()
                     AEval( aRaport[ 'ListaPom' ], { | aPoz |
                        cRaport += "Nr dokumentu: " + AllTrim( aPoz[ 'Importowany' ][ 'K_3A' ] ) + hb_eol()
                        cRaport += "Kontrahent: " + AllTrim( aPoz[ 'Importowany' ][ 'K_5A' ] ) + hb_eol()
            ...
            CASE nMenu == 3
               ...
               @ 10, 17 SAY "Zezw¢l na import nieaktywnych dokument¢w" GET cPuste PICTURE "!" VALID ValidTakNie( cPuste, 12, 59 )
               @ 12, 52 GET lOk PUSHBUTTON CAPTION ' Zamknij ' STATE { || ReadKill( .T. ) }
               ValidTakNie( cPuste, 12, 59 )
               READ
               IF LastKey() <> K_ESC
                  aDane[ 'ZezwolNaDuplikaty' ] := cTN
                  aDane[ 'OpisZd' ] := cOpisZd
...
FUNCTION JPKImp_PKPIR_Wczytaj( cPlikJpk )
   ...
   IF File( cPlikJpk )
      ...
      IF oDoc:nError == HBXML_ERROR_NONE
         ...
         aFirma := PobierzFirme( Val( ident_fir ) )
         aDaneJPK[ 'Firma' ] := aFirma
         aWeryf := JPKImp_Weryf( @aDaneJPK, TrimNip( aFirma[ 'firma' ][ 'nip' ] ), Val( param_rok ), , 4 )
         ...
         IF aWeryf[ 'OK' ]
            ...
            IF Len( aDaneJPK[ 'Wiersze' ] ) > 0
               ...
               AEval( aDaneJPK[ 'Wiersze' ], { | aW |
                  ...
                  IF hb_HHasKey( aW, 'K_3' )
                     aW[ 'K_3A' ] := sxmlTrim( aW[ 'K_3' ] )
                  ELSE
                     aW[ 'K_3A' ] := sxmlTrim( xmlWartoscH( aW, 'K_3A' ) )
                  ENDIF
                  aW[ 'K_3B' ] := sxmlTrim( xmlWartoscH( aW, 'K_3B' ) )
                  aW[ 'K_4A' ] := sxmlTrim( xmlWartoscH( aW, 'K_4A' ) )
                  aW[ 'K_4B' ] := sxmlTrim( xmlWartoscH( aW, 'K_4B' ) )
                  IF hb_HHasKey( aW, 'K_4' )
                     aW[ 'K_5A' ] := sxmlTrim( aW[ 'K_4' ] )
                  ELSE
                     aW[ 'K_5A' ] := sxmlTrim( xmlWartoscH( aW, 'K_5A' ) )
                  ENDIF
                  IF hb_HHasKey( aW, 'K_5' )
                     aW[ 'K_5B' ] := sxmlTrim( aW[ 'K_5' ] )
                  ELSE
                     aW[ 'K_5B' ] := sxmlTrim( xmlWartoscH( aW, 'K_5B' ) )
                  ENDIF
                  ...
                  ELSE
                     aW[ 'K_9' ] := 0
                  ENDIF
                  IF hb_HHasKey( aW, 'K_9A' ) .AND. HB_ISCHAR( aW[ 'K_9A' ] )
                     aW[ 'K_9A' ] := sxml2num( aW[ 'K_9A' ], 0 )
                     aDaneJPK[ 'WierszeSum' ][ 'K_9A' ] += aW[ 'K_9A' ]
                  ELSE
                     aW[ 'K_9A' ] := 0
                  ENDIF
                  IF hb_HHasKey( aW, 'K_10' ) .AND. HB_ISCHAR( aW[ 'K_10' ] )
                  ...
                  ELSE
                     aW[ 'K_14' ] := 0
                  ENDIF
                  IF hb_HHasKey( aW, 'K_14A' ) .AND. HB_ISCHAR( aW[ 'K_14A' ] )
                     aW[ 'K_14A' ] := sxml2num( aW[ 'K_14A' ], 0 )
                     aDaneJPK[ 'WierszeSum' ][ 'K_14A' ] += aW[ 'K_14A' ]
                  ELSE
                     aW[ 'K_14A' ] := 0
                  ENDIF
                  IF hb_HHasKey( aW, 'K_15' ) .AND. HB_ISCHAR( aW[ 'K_15' ] )
...
PROCEDURE JPKImp_PKPIR_Podglad( aDane, aSumy )
   ...
   LOCAL nElem := 1
   LOCAL aNaglowki := { "Import", "K_1", "K_2", "K_3A", "K_3B", "K_4A", "K_4B", "K_5A", "K_5B", "K_6", "K_7", ;
      "K_8", "K_9", "K_9A", "K_10", "K_11", "K_12", "K_13", "K_14", "K_14A", "K_15", "K_16A", "K_16B", "K_17" }

   LOCAL aBlokiKolumn := { ;
      { || iif( aDane[ nElem ][ "Importuj" ], "Tak", "Nie" ) }, ;
      { || PadC( aDane[ nElem ][ "K_1" ], 6 ) }, ;
      { || DToS( aDane[ nElem ][ "K_2" ] ) }, ;
      { || PadR( aDane[ nElem ][ "K_3A" ], 16 ) }, ;
      { || PadR( aDane[ nElem ][ "K_3B" ], 35 ) }, ;
      { || PadR( aDane[ nElem ][ "K_4A" ], 2 ) }, ;
      { || PadR( aDane[ nElem ][ "K_4B" ], 16 ) }, ;
      { || PadR( aDane[ nElem ][ "K_5A" ], 25 ) }, ;
      { || PadR( aDane[ nElem ][ "K_5B" ], 25 ) }, ;
      ...
      { || iif( hb_HHasKey( aDane[ nElem ], "K_9A" ), Transform( aDane[ nElem ][ "K_9A" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_10" ), Transform( aDane[ nElem ][ "K_10" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_11" ), Transform( aDane[ nElem ][ "K_11" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_12" ), Transform( aDane[ nElem ][ "K_12" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_13" ), Transform( aDane[ nElem ][ "K_13" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_14" ), Transform( aDane[ nElem ][ "K_14" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_14A" ), Transform( aDane[ nElem ][ "K_14A" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_15" ), Transform( aDane[ nElem ][ "K_15" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_16A" ), PadR( aDane[ nElem ][ "K_16A" ], 20 ), Space( 20 ) ) }, ;
      { || iif( hb_HHasKey( aDane[ nElem ], "K_16B" ), Transform( aDane[ nElem ][ "K_16B" ], RPICE ),  Transform( 0, RPICE ) ) }, ;
   ...
   LOCAL aStopki := { "", "", "", "", "", "", "", "", "", "", Transform( aSumy[ "K_7" ], RPICE ), ;
      ...
      Transform( aSumy[ "K_9A" ], RPICE ), ;
      Transform( aSumy[ "K_10" ], RPICE ), Transform( aSumy[ "K_11" ], RPICE ), ;
      Transform( aSumy[ "K_12" ], RPICE ), Transform( aSumy[ "K_13" ], RPICE ), ;
      Transform( aSumy[ "K_14" ], RPICE ), Transform( aSumy[ "K_14A" ], RPICE ), ;
      Transform( aSumy[ "K_15" ], RPICE ), ;
...
PROCEDURE JPKImp_Rycz()

   LOCAL aDane := hb_Hash( 'ZezwolNaDuplikaty', 'N', 'OpisZd', Space( 30 ), ;
      'ZezwolNaPuste', 'N', 'Kolumna', 5, 'DataKsg', param_kssk )
   LOCAL cPlik
   LOCAL cKolor
   LOCAL cEkran := SaveScreen()
   LOCAL nMenu, cEkran2
   LOCAL aRaport, cRaport, cTN, lOk, cPuste, nKolumna, cDataKsg
   LOCAL nSumaImp, nLiczbaLp := 0
   LOCAL bKolumnaW := { ||
      LOCAL cKolor := ColInf()
      @ 24, 0 SAY PadC( Tab_RyczInfo(), 80 )
   ...
   LOCAL bKolumnaV := { ||
      LOCAL lRes := nKolumna >= 5 .AND. nKolumna <= 13
      IF lRes
         @ 24, 0
      ENDIF
      RETURN lRes
   ...
   IF HB_ISHASH( aDane[ 'JPK' ] )
      ...
      aDane[ 'Dekret' ] := KosImp_VatS_Dekretuj( aDane, .T. )
      nLiczbaLp := Len( aDane[ 'JPK' ][ 'Pozycje' ] )
      ...
      PrintTextEx( 12, 4, "  NIP firmy: {w+}" + aDane[ 'JPK' ][ 'Podmiot' ][ 'NIP' ] )
      PrintTextEx( 14, 4, "Liczba pozycji w pliku JPK: {w+}" + AllTrim( Str( nLiczbaLp ) ) )
      PrintTextEx( 15, 4, PadR( "Liczba importowanych pozycji: {w+}" + AllTrim( Str( Len( aDane[ 'JPK' ][ 'Pozycje' ] ) ) ), 72 ) )
      ...
      DO WHILE nMenu != 0
         ...
         CASE nMenu == 1
            IF TNEsc( , "Czy wykona† import danych sprzeda¾y? ( T / N )" )
               ...
               aRaport := JPKImp_Rycz_Importuj( aDane )
         ...
         CASE nMenu == 3
            ...
            cOpisZd := aDane[ 'OpisZd' ]
            cPuste := aDane[ 'ZezwolNaPuste' ]
            nKolumna := aDane[ 'Kolumna' ]
            cDataKsg := aDane[ 'DataKsg' ]
            @  6, 13 CLEAR TO 16, 71
            @  7, 15 TO 15, 69
            @  8, 17 SAY "Zezw¢l na import dokument¢w z istniej¥cym nr" GET cTN PICTURE "!" VALID cTN$"TN"
            @  9, 17 SAY "Opis zdarzenia" GET cOpisZd VALID JPKImp_VatS_Tresc_V( "O" )
            @ 10, 17 SAY "Zezw¢l na import nieaktywnych dokument¢w" GET cPuste PICTURE "!" VALID ValidTakNie( cPuste, 10, 59 )
            @ 11, 17 SAY "Domy˜lna kolumna" GET nKolumna PICTURE "99" WHEN Eval( bKolumnaW ) VALID Eval( bKolumnaV )
            @ 12, 17 SAY "Do ksi©gi na dzieä (S-sprzed., W-wystaw., K-KSeF)" GET cDataKsg PICTURE "!" VALID cDataKsg $ 'KSW'
            @ 14, 52 GET lOk PUSHBUTTON CAPTION ' Zamknij ' STATE { || ReadKill( .T. ) }
            ValidTakNie( cPuste, 10, 59 )
            READ
            IF LastKey() <> K_ESC
               aDane[ 'ZezwolNaDuplikaty' ] := cTN
               aDane[ 'OpisZd' ] := cOpisZd
               aDane[ 'ZezwolNaPuste' ] := cPuste
               aDane[ 'Kolumna' ] := nKolumna
               aDane[ 'DataKsg' ] := cDataKsg
            ENDIF
            RestScreen( , , , , cEkran2 )
         CASE nMenu == 4
...
FUNCTION JPKImp_Rycz_Importuj( aDane )
   ...
   AEval( aDane[ 'Dekret' ], { | aPoz |
      ...
      LOCAL aIstniejacyRec, nKolimna, nWartosc
      LOCAL lImportuj := aPoz[ 'FakturaPoz' ][ 'Importuj' ]
      ...
      IF lImportuj
         ...
         zTRESC := aDane[ 'OpisZd' ]
         zKRAJ := aPoz[ 'zkraj' ]
         zDATAPRZY := aPoz[ 'zdatas' ]
         zNRKSEF := aPoz[ 'NrKSeF' ]
         zZAPLATA := '1'

         nKolumna := Val( aPoz[ 'zkolumna' ] )
         IF nKolumna == 0
            nKolumna := aDane[ 'Kolumna' ]
         ENDIF
         ...
         nWartosc := aPoz[ 'zwart02' ] + aPoz[ 'zwart07' ] + aPoz[ 'zwart22' ] + aPoz[ 'zwart00' ] + aPoz[ 'zwart08' ]
         ...
         IF aDane[ 'ZezwolNaDuplikaty' ] == 'N' .AND. EwidSprawdzNrDokRec( 'EWID', ident_fir, miesiac, znumer, @aIstniejacyRec )
            aRaport[ 'Pominieto' ] := aRaport[ 'Pominieto' ] + 1
            AAdd( aRaport[ 'ListaPom' ], hb_Hash( 'Istniejacy', aIstniejacyRec, 'Importowany', aPoz, 'Przyczyna', 'Istnieje ju¾ dokument o tym numerze' ) )
         ELSE
            //Rycz_Ksieguj()
