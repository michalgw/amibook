...
*--------------------------------------
   ...
   *            +iif(SPZW='P',WARTZW,0)
      ...
      *@@@@@@@@@@@@@@@@@@@@@@ MODUL OBLICZEN @@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      ENDIF
      IF Empty( P6a )
         Komunikat( "Brak kodu urz©du.;Prosz© uzupeàniÜ lub wybraÜ odpowiedni urz•d skarbowy" )
         BREAK
      ENDIF
      p8p := ''
      IF spolka_
...
*            wait 'P8i...'+p8i+'...'
         ELSE
            ...
            P8p := ''
            P11 := Space( 10 )
            Komunikat( "Prosz© wybraÜ nazwisko peànomocnika lub wàaòciciela w informacji o firmie." )
            BREAK
         ENDIF
      ENDIF
      IF spolka_
...
FUNCTION JPK_V7_DaneDek( aDane )
   ...
   hDane['P_31'] := _round( SEK_CV7net + p65dekwe, 0 )
   hDane['P_32'] := _round( SEK_CV7vat + p65vdekwe, 0 )
...
FUNCTION VAT9M_DaneDek()

   LOCAL aDane := {=>}

   aDane[ 'P_10' ] := _round( P65dekus, 0 )
   aDane[ 'P_11' ] := _round( P65vdekus, 0 )
   aDane[ 'P_12' ] := _round( P65dekusu, 0 )
   aDane[ 'P_13' ] := _round( P65vdekusu, 0 )
   aDane[ 'P_14' ] := _round( SEK_CV7net + p65dekwe, 0 )
   aDane[ 'P_15' ] := _round( SEK_CV7vat + p65vdekwe, 0 )
   aDane[ 'P_16' ] := 0
   aDane[ 'P_17' ] := 0
