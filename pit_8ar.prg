...
begin sequence
   ...
   if dostepex('TABPIT8R')
      zap
      inde on del+firma+mc to TABPIT8R
   else
      break
   endif
...
*****************************
****** te kwoty beda rozbijane miesiecznie w TABPIT8R
*   p29=p29+zP4IL_POD
*   p30=p30+zP4SUM_WYP
*   p31=p31+zP4SUM_ZAL
*********************************
...
*****************************
   ...
   case _OU='P'
      ...
      rl('(01) Numer identyfikacyjny podatnika : '+P1)
      rl('(04) Za rok : '+P3+'.'+P4)
      rl()
   ...
   CASE _OU='X'
        edeklaracja_plik = 'PIT_8AR_8_' + normalizujNazwe(AllTrim(symbol_fir)) + '_' + AllTrim(p4r)
        IF ( zCzyKorekta := edekCzyKorekta() ) > 0
           ...
           IF zDEKLKOR != 'K' .OR. (zDEKLKOR == 'K' .AND. ValType(tresc_korekty_pit8ar) == "C")
              PRIVATE cWynikXml := ''
              cWynikXml = edek_pit8ar_8()
              edekZapiszXml(cWynikXml, edeklaracja_plik, wys_edeklaracja, 'PIT8AR-8', zDEKLKOR == 'K')
           ENDIF
        ENDIF
   other //_OU='K'
        DeklPodp( 'T' )
