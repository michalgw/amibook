...
PROCEDURE FaPoz()
   ...
   PRIVATE _row_g, _col_l, _row_d, _col_p, _invers, _curs_l, _curs_p, _esc, _top, _bot, _stop, _sbot, ;
      _proc, _row, _proc_spe, _disp, _cls, kl, ins, nr_rec, wiersz, f10, rec, fou, _top_bot
   *################################# GRAFIKA ##################################
   ColStd()
   @  1,47 SAY '          '
   @  8, 0 CLEAR TO 22, 79
      ...
      CASE kl == K_INS .OR. kl == Asc( '0' ) .OR. _row == -1 .OR. kl == Asc( 'M' ) .OR. kl == Asc( 'm' )
         ...
         DO WHILE .T.
            ...
            *ðððððððððððððððððððððððððððððððð GET ðððððððððððððððððððððððððððððððððð
            @ wiersz,  1 GET zTOWAR PICTURE '@S29 ' + repl( 'X', 40 ) WHEN w26_50v()
            @ wiersz, 31 GET zSWW   PICTURE '@RS8 !!!!!!!!!!!!!!'
            IF NR_UZYTK == 204
               @ wiersz, 40 GET zILO PICTURE '999999.99' VALID vWARTOSCf()
            ELSE
               @ wiersz, 40 GET zILO PICTURE '99999.999' VALID vWARTOSCf()
            ENDIF
            @ wiersz, 50 GET zJM    PICTURE 'XXXXX'
            @ wiersz, 56 GET zCENA  PICTURE '999999.99' VALID vWARTOSCf()
            @ wiersz, 77 GET zSTAWKA PICTURE '!!' VALID vSTAWKAf()
            read_()
            IF LastKey() == K_ESC
               EXIT
            ENDIF
