...
PROCEDURE FakturyN()
   ...
   PRIVATE aKsiegWybor := { "1. Zaksi©guj w bieæ•cym miesi•cu", ;
            ...
            BEGIN SEQUENCE
               *-------zamek-------
               ...
               IF ! ins .AND. faktury->korekta <> 'T' .AND. faktury->dokkorid <> 0
                  kom( 3, '*u', '  Nie moæna modyfikowaÜ. Ta faktura posiada korekt©.  ' )
                  BREAK
               ENDIF
               IF ! ins .AND. ( ! Empty( faktury->ksefnrksef ) .OR. ( ! Empty( faktury->ksefnrses ) .AND. faktury->ksefstatus < 400 ) )
                  Komun( 'Nie moæna modyfikowaÜ. Faktura zostaàa wysàana do KSeF.' )
                  BREAK
               ENDIF
               *-------------------
               ...
               * ZMIENNE 
               IF ins .AND. kl == K_F6
                  ...
                  IF ! Empty( aBufDok ) .AND. HB_ISHASH( aBufDok )
                     ...
                     IF lKorekta
                        @ 22, 29 SAY 'WARTOóè KOREKTY¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒ¡ƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒŸ'
                     ENDIF
               ...
               ELSEIF ins
                  ...
                  @ 15, 59 CLEAR TO 21, 67
                  @ 15, 69 CLEAR TO 21, 78
                  @ 22, 29 SAY '               ¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒ¡ƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒŸ'
            ...
            BEGIN SEQUENCE
               *-------zamek-------
               ...
               IF faktury->korekta <> 'T' .AND. faktury->dokkorid <> 0
                  kom( 3, '*u', ' Nie moæna usun•Ü. Ta faktura posiada korekt©. ' )
                  BREAK
               ENDIF
               IF ! Empty( faktury->ksefnrksef ) .OR. ( ! Empty( faktury->ksefnrses ) .AND. faktury->ksefstatus < 400 )
                  Komun( 'Nie moæna usun•Ü. Faktura zostaàa wysàana do KSeF.' )
                  BREAK
               ENDIF
               *-------------------
         ...
         CASE kl == K_ENTER .OR. kl == Asc( 'W' ) .OR. kl == Asc( 'w' )
            ...
            BEGIN SEQUENCE
               IF kl == Asc( 'W' ) .OR. kl == Asc( 'w' )
                  IF ! Empty( faktury->ksefnrksef ) .OR. ( ! Empty( faktury->ksefnrses ) .AND. faktury->ksefstatus < 400 )
                     Komun( 'Faktura zostaàa wysàana do KSeF.' )
                     BREAK
                  ENDIF
                  IF ! TNEsc( , "Czy wysàaÜ faktur© do KSeF? (Tak/Nie)" )
                     BREAK
                  ENDIF
               ENDIF
    ...
    *             @ 21, 6 SAY ODBOSOBA
                  ...
                  ELSE
                     IF FakturyN_TworzFA3()
                        ...
                        IF ! Empty( oStatus )
                           ...
                           Komun( "Faktura zostaàa wysàana do KSeF" )
                        ELSE
                           Komun( "Nie udaào si© wysàaÜ faktury do KSeF" )
                        ENDIF
                     ENDIF
                  ENDIF
         ...
         CASE kl == Asc( 'S' ) .OR. kl == Asc( 's' )
            ...
            IF ! Empty( faktury->ksefnrses ) .AND. ! Empty( faktury->ksefnrele )
               IF Empty( faktury->ksefnrksef )
                  IF faktury->ksefstatus < 200
                     ...
                     IF ! Empty( oStatus )
                        ...
                        FakturyN_StatusKSeF()
                     ELSE
                        Komun( "Nie udaào si© pobraÜ statusu KSeF" )
                     ENDIF
                  ELSE
                     FakturyN_StatusKSeF()
                  ENDIF
               ...
               ENDIF
            ELSE
               Komun( "Faktura nie zostaàa wysàana do KSeF" )
            ENDIF
         ...
         *################################### POMOC ##################################
         ...
         CASE kl == K_F5
            IF faktury->korekta <> 'T'
               ...
               ENDIF
               Komun( "Dokument zostaà skopiowany" )
            ELSE
               Komun( "Nie moæna kopiowaÜ faktury koryguj•cej" )
            ENDIF

         CASE kl == K_SH_F5
            IF TNEsc( , "Czy skopiowaÜ wszytkie faktury do bufora? (Tak/Nie)" )
               nAktRec := RecNo()
               nLicznik := 0
               GO TOP
               SEEK "+" + ident_fir + miesiac
         ...
         CASE kl == K_CTRL_F10
            IF faktury->korekta <> 'T' .AND. faktury->dokkorid <> 0
               ...
               IF aTmpRec[ 'KOREKTA' ] <> 'T' //.AND. aTmpRec[ 'DOKKORID' ] == faktury->( RecNo() )
                  IF TNEsc( , "Czy odblokowaÜ faktur©? (Tak/Nie)" )
                     BlokadaR()
                     faktury->dokkorid := 0
                     faktury->( dbCommit() )
                     faktury->( dbUnlock() )
                  ENDIF
               ELSE
                  Komun( "Nie moæna odblokowaÜ tej faktury." )
               ENDIF
            ELSE
               Komun( "Nie moæna odblokowaÜ tej faktury." )
            ENDIF
         ...
         CASE kl == K_F7
            IF HB_ISHASH( bufor_dok ) .AND. hb_HHasKey( bufor_dok, 'faktury' ) .AND. HB_ISARRAY( bufor_dok[ 'faktury' ] ) .AND. Len( bufor_dok[ 'faktury' ] ) > 0
               ...
               IF TNEsc( , "Czy wstawiÜ wsystkie faktury z bufora? (Tak/Nie)" )

               ins := .T.
               lKorekta := .F.
               ColInf()
               @ 24, 0 SAY PadC( "Dodawanie dokument¢w... Prosz© czekaÜ...", 80 )
...
PROCEDURE say260vn()
   ...
   *  set cent off
   ...
   ColStd()
   @ 22, 29 SAY iif( KOREKTA == 'T', 'WARTOóè KOREKTY', '               ' ) + '¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒ¡ƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒŸ'
   @ 23, 0 SAY 'Kontrola zaplat....  Termin zaplaty.... (..........) Juz zaplacono.             '
   SET COLOR TO +w
...
FUNCTION wfSplitPay()
   ...
   ColInf()
   @ 24, 0 SAY PadC( 'Mechanizm podzielonej pàatnoòci (split payment): T-tak   lub   N-nie', 80, ' ' )
   ColStd()
   ...
   IF Found()
      ...
      IF zRYCZALT <> 'T'
         ************* ZAPIS REJESTRU DO KSIEGI *******************
         SELECT oper
         IF faktury->KSGZBIOR == 'N'
            SET ORDER TO 5
            SEEK '+' + Str( ksREKZAK, 5 ) + 'F-'
            IF Found()
               BlokadaR()
               zWartoscBylo := WYR_TOW
               DELETE
               COMMIT
               UNLOCK
               SET ORDER TO 1
               ...
               *******************************
               ...
               BlokadaR()
               repl_( 'wyr_tow', wyr_tow - zWartoscBylo )
...
FUNCTION FakturyN_Dane()
   ...
   IF ! Empty( faktury->datas )
      ...
      CASE faktury->data2typ == 'U'
         aDane[ 'data_rodzaj' ] := 'Data wykonania usàugi'
   ...
   IF faktury->splitpay == 'T'
      aDane[ 'dopisek' ] := 'Mechanizm podzielonej pàatnoòci'
      aDane[ 'plitpayt' ] := .T.
   ELSE
      aDane[ 'dopisek' ] := ''
      aDane[ 'plitpay' ] := .F.
...
PROCEDURE FakturyN_KorInfo()
   ...
   cKolor := ColSta()
   @  2, 0 SAY 'ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ'
...
PROCEDURE FakturyN_RysujTlo()
   ...
   @  8, 0 SAY '⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒø'
   @  9, 0 SAY '≥         Nazwa towaru/us&_l.ugi          ≥  Ilo&_s.&_c.  ≥ JM  ≥Cena nett≥Wart.netto≥VA≥'
   @ 10, 0 SAY '√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ≈ƒƒ¥'
   @ 11, 0 SAY '≥                                      ≥         ≥     ≥         ≥          ≥  ≥'
   @ 12, 0 SAY '≥                                      ≥         ≥     ≥         ≥          ≥  ≥'
   @ 13, 0 SAY '≥                                      ≥         ≥     ≥         ≥          ≥  ≥'
   @ 14, 0 SAY '¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒ¬ƒƒƒƒ¡ƒƒƒƒƒ≈ƒƒ¬ƒƒƒƒƒƒ¡ƒƒ¬ƒƒƒƒƒƒƒ¡ƒƒ¥'
   ...
   @ 20, 0 SAY 'Adres.                                      ≥          ≥ZW≥         ≥          ≥'
   @ 21, 0 SAY '                                       RAZEM≥          ≥  ≥         ≥          ≥'
   @ 22, 0 SAY 'Forma pàat.                                 ¿ƒƒƒƒƒƒƒƒƒƒ¡ƒƒ¡ƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒŸ'
   @ 23, 0 SAY 'Kontrola zaplat....  Termin zaplaty.... (..........) Juz zaplacono.             '
   *if NR_UZYTK=800
   *   @ 20,0  say 'Opl.skarb.'
   *   @ 21,0  say 'Od darowiz.'
...
FUNCTION FakturyN_TworzFA3()
   ...
   */
   ...
   cFaktura += '      <StopkaFaktury>Kapiaë? zakë?adowy 5 000 000</StopkaFaktury>' + nl
   cFaktura += '    </Informacje>' + nl
...
PROCEDURE FakturyN_StatusKSeF()
   ...
   IF ! Empty( faktury->ksefnrses ) .AND. ! Empty( faktury->ksefnrele )
      ...
      @ 13, 2 SAY "Szczeg¢ày: " + AllTrim( SubStr( faktury->ksefstszcz, 1, 70 ) )
