...
PROCEDURE Vat_717( _G, _M, _STR, _OU )
   ...
   *#################################     VAT_7      #############################
   BEGIN SEQUENCE
      ...
      IF _STR == 1 .OR. _STR == 2
         ...
         SET COLOR TO w+
         @  9, 42 SAY ' '+padc('D.ROZLICZENIE PODATKU NALICZONEGO', 37, 'ƒ' )
         ColStd()
         ...
         @ 13, 42 SAY '                                      '
         SET COLOR TO w+
         @ 14, 42 SAY ' ' + PadC( 'E.OBLICZENIE WYSOKO&__S.CI ZOBOWI&__A.Z.', 37, 'ƒ' )
         ColStd()
         ...
         ENDIF
         SET COLOR TO w+
         @ 18, 42 say ' ' + PadC( '(gdy nadwy&_z.ka podatku naliczonego)', 37, 'ƒ' )
         ColStd()
...
*        ColStd()
         ...
         @  3, 70 GET kordek PICTURE '!' WHEN wfkordek17( 3, 71 ) VALID vfkordek17( 3, 71 )
         IF zVATFORDR == '7D'
...
*            +iif(SPZW='P',WARTZW,0)
      ...
      p76 := ( p62 + p62a + p70 + p72 + p65vdekue + p65vdekit + p65vdekus + p65vdekwe + pp12 + p65vdekusu + art111u6 ) - znowytran
   ...
   *             endif
      ...
      CASE _OU == 'K'
         ...
         CASE 1
            SWITCH zVATFORDR
            CASE '7 '
               DeklarDrukuj( 'VAT7-17' )
               EXIT
            CASE '7K'
               DeklarDrukuj( 'VAT7K-11' )
               EXIT
            CASE '7D'
...
//         IF ValType( tresc_korekty_vat7 ) == "C"
            ...
            DO CASE
            CASE zVATFORDR == '7 '
               resdekl := edek_vat7_17( aDane )
               edekZapiszXml( resdekl, edeklaracja_plik, wys_edeklaracja, 'VAT7-17', kordek == 'K', Val( miesiac ) )
            CASE zVATFORDR == '7K'
               resdekl := edek_vat7k_11( aDane )
               edekZapiszXml( resdekl, edeklaracja_plik, wys_edeklaracja, 'VAT7K-11', kordek == 'K', Val( miesiac ) )
            CASE zVATFORDR == '7D'
