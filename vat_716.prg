...
PROCEDURE Vat_716( _G, _M, _STR, _OU )
   ...
   *#################################     VAT_7      #############################
   BEGIN SEQUENCE
      ...
      IF _STR == 1 .OR. _STR == 2
         ...
         @ 21, 42 SAY '    w tym w terminie  25 dni          '
         @ 22, 42 SAY ' 60 dni              180 dni          '
*        set colo to w+
*        @ 22,42 to 22,79
*        ColStd()

         @  3, 70 GET kordek PICTURE '!' WHEN wfkordek16( 3, 71 ) VALID vfkordek16( 3, 71 )
         IF zVATFORDR == '7D'
         ...
         REPLACE VATZALMIE WITH zVATZALMIE, VATNADKWA WITH zVATNADKWA
         ...
         DO WHILE dane_vat <> 1
            ...
            @ 20, 42 PROMPT '      INFORMACJA O PODATKU VAT        '
            @ 21, 42 PROMPT 'CZYNNOSCI WYKONYWANE W OKRESIE (sek.F)'
            @ 22, 42 PROMPT ' WYLICZENIE STRUKTURY SPRZEDAZY ' + Str( Val( param_rok ) - 1, 4 ) + 'r.'
            MENU TO dane_vat
            ColStd()
            IF LastKey() == K_ESC
               BREAK
...
*--------------------------------------
      ...
      *@@@@@@@@@@@@@@@@@@@@@@ MODUL OBLICZEN @@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      STORE 0 TO art129, zzwr60dni, zzwr25dni, zVATZALMIE, zVATNADKWA, p98doprze
...
*            +iif(SPZW='S',WARTZW,0)
            p47 := p47 + iif( SP02 == 'S' .AND. ZOM02 == 'M' .AND. VAT02 <> 0, WART02, 0 ) + ;
               iif( SP07 == 'S' .AND. ZOM07 == 'M' .AND. VAT07 <> 0, WART07, 0 ) + ;
               iif( SP22 == 'S' .AND. ZOM22 == 'M' .AND. VAT22 <> 0, WART22, 0 ) + ;
               iif( SP12 == 'S' .AND. ZOM12 == 'M' .AND. VAT12 <> 0, WART12, 0 ) + ;
...
*            +iif(SPZW='P',WARTZW,0)
            p49 := p49 + P49tmp
            P51tmp := iif( SP02 == 'P' .AND. ZOM02 == 'M' .AND. VAT02 <> 0, WART02, 0 ) + ;
               iif( SP07 == 'P' .AND. ZOM07 == 'M' .AND. VAT07 <> 0, WART07, 0 ) + ;
               iif( SP22 == 'P' .AND. ZOM22 == 'M' .AND. VAT22 <> 0, WART22, 0 ) + ;
               iif( SP12 == 'P' .AND. ZOM12 == 'M' .AND. VAT12 <> 0, WART12, 0 ) + ;
      ...
      p76 := ( p62 + p62a + p70 + p72 + p65vdekue + p65vdekit + p65vdekus + p65vdekwe + pp12 + p65vdekusu ) - znowytran

      p79 := pp8 + pp11 + p46dek + p50dek + zkorekst + zkorekpoz
...
*            wait 'P8i...'+p8i+'...'
   ...
   *             endif
      ...
      CASE _OU == 'P'
         ...
         rl( PadC( '================================', 80 ) )
         rl(                                          PadL( 'ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿', 80, ' ' ) )
         rl(                                          PadL( '³Podstawa opoda-³    Podatek    ³', 80, ' ' ) )
         rl(                                          PadL( '³ tkowania w zl ³ nale&_z.ny w z&_l.  ³', 80, ' ' ) )
         rl(                                          PadL( 'ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´', 80 ) )
         ...
         rl(                                          PadL( 'ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´', 80, ' ' ) )
         ...
         rl(                                          PadL( 'ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´', 80, ' ' ) )
         rl( 'RAZEM                                          ³' + kwota( P75, 14, 0 ) + ' ³' + kwota( P76, 14, 0 ) + ' ³' )
         rl(                                          PadL( 'ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ', 80, ' ' ) )
         rl()
         rl( PadC( 'D. ROZLICZENIE PODATKU NALICZONEGO', 80 ) )
         rl( PadC( '==================================', 80 ) )
         rl( 'D.1. PRZENIESIENIA' + Space( 45 ) +                'ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿' )
         rl( '------------------' + Space( 45 ) +                '³  Podatek do   ³' )
         rl(                                               PadL( '³odliczenia w z&_l.³', 80, ' ' ) )
         rl(                                               PadL( 'ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´', 80, ' ' ) )
         ...
         rl( '                                               ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´' )
      ...
      CASE _OU == 'K'
         ...
         CASE 1
            SWITCH zVATFORDR
            CASE '7 '
               DeklarDrukuj( 'VAT7-16' )
               EXIT
            CASE '7K'
               DeklarDrukuj( 'VAT7K-10' )
               EXIT
            CASE '7D'
               DeklarDrukuj( 'VAT7D-7' )
               EXIT
            ENDSWITCH
            EXIT
         CASE 2
      ...
      CASE _OU == 'X'
         ...
         IF ValType( tresc_korekty_vat7 ) == "C"
            ...
            DO CASE
            CASE zVATFORDR == '7 '
               resdekl := edek_vat7_16()
               edekZapiszXml( resdekl, edeklaracja_plik, wys_edeklaracja, 'VAT7-16', kordek == 'K', Val( miesiac ) )
            CASE zVATFORDR == '7K'
               resdekl := edek_vat7k_10()
               edekZapiszXml( resdekl, edeklaracja_plik, wys_edeklaracja, 'VAT7K-10', kordek == 'K', Val( miesiac ) )
            CASE zVATFORDR == '7D'
               resdekl := edek_vat7d_7()
               edekZapiszXml( resdekl, edeklaracja_plik, wys_edeklaracja, 'VAT7D-7', kordek == 'K', Val( miesiac ) )
            ENDCASE
         ENDIF
      CASE _OU == 'I'
         infov716()
