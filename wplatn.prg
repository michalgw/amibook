...
*          zOPI_PLA=padr(alltrim(zNAZWA_PLA)+' '+alltrim(zULICA_PLA)+' '+alltrim(zMIEJSC_PLA),114)
*          zOPI_PLA=iif(zPODATKI,rozrzut(zOPI_PLA),zOPI_PLA)
*          zOPI_WIE=padr(alltrim(zNAZWA_WIE)+' '+alltrim(zULICA_WIE)+' '+alltrim(zMIEJSC_WIE),104)
*          zOPI_WIE=iif(zPODATKI,rozrzut(zOPI_WIE),zOPI_WIE)
*          zzTRESC=iif(zPODATKI,rozrzut(zTRESC),padc(zTRESC,54))
...
*           endif
           rl()
           rl(space(_M)+space(1)+padr('**'+alltrim(str(zKWOTA,12,2)),19)+space(5)+upper(substr(alltrim(slownie(zKWOTA)),1,52)))
*           if len(alltrim(zKONTO_PLA))<=29
*              rl(space(_M)+space(1)+padr('**'+alltrim(str(zKWOTA,12,2)),19)+space(5)+zKONTO_PLA)
*           else
...
*           endif
   ...
   case _OU='P'
      rl()
      rl(padc('DOWOD ISTNIEJE TYLKO W FORMIE "DO ZADRUKOWANIA". BRAK PODGLADU',80))
      rl(padc('==============================================================',80))
   endcase
end
*close_()
