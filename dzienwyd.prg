...
PROCEDURE DzienWyd( zzadzien, zzestnr )
   ...
   BEGIN SEQUENCE
      ...
      *@@@@@@@@@@@@@@@@@@@@@@@@@@ ZAKRES @@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      nMenu := GraficznyCzyTekst( "DzienWyd" )
      IF nMenu == 0
         BREAK
      ENDIF
      ...
      IF nMenu == 1
         aDane := { ;
            ...
            'ulica' => AllTrim( firma->ulica ), ;
            'nr_domu' => AllTrim( firma->nr_domu ), ;
            'nr_mieszk' => AllTrim( firma->nr_mieszk ), ;
            'uzytkownik' => code(), ;
            'zestnr' => AllTrim( zestnr ), ;
            'zadzien'  => zadzien, ;
            'miesiac' => AllTrim( Upper( Miesiac( Val( miesiac ) ) ) ), ;
            'rok' => param_rok, ;
            'stawka_a' => vat_A, ;
            'stawka_b' => vat_B, ;
            'stawka_c' => vat_C, ;
         ...
         DO WHILE .NOT. &_koniec
            ...
            DO WHILE .NOT. &_bot .AND. dzien == zadzien .AND. numer == zestnr .AND. numer_rach = k3 .AND. .NOT. Eof()
               ...
               ENDCASE
               aPoz[ 'netto' ] += netto
               aPoz[ 'wartvat' ] += wartvat
               SKIP 1
            ENDDO

            AAdd( aDane[ 'pozycje' ], aPoz )
         ENDDO
         FRDrukuj( 'frf\dzienwyd.frf', aDane )
      ELSE
         *@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
         mon_drk( 'ö' + ProcName() )
         IF _mon_drk == 2
